...
function createGaugeChart(sample) {
  ...
  d3.json("samples.json").then(data =>{
    ...
    //console.log("createGaugeChart matchedSampleObj", matchedSampleObj);
...
d3.json("samples.json").then((data) => {
  ...
  // Use the first sample from the list to create the initial plots
  const firstSample = sampleNames[0];
  createMetadata(firstSample);
  createCharts(firstSample);
  createGaugeChart(firstSample)
